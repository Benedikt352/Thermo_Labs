...
for i in range(1, nstep):
    ...
    r = 4 * np.random.rand(1, 1)
     # Temperature difference to the surrounding
    DT_t = Tt[i - 1] - Tr
    DT_b = Tb[i - 1] - Tr
    l = iso*np.random.rand() #generate random value form 0 until iso, all equally probable. The higher iso, the lower
                            #the chance that the if statements that mimic heat loss to the environment are executed, so 
                            #a higher iso value mimics a better isolation of the bodies

    if l > 0 and l < 1:
        
        if r < DT_t:
            # Move heat quanta from top away
            Tt[i] = Tt[i] - 1 / N 
...
plt.ylabel(r'$2(T-<T_0>)/\Delta T_0$')
plt.legend(['Temperature bottom', 'Temperature top'])
plt.savefig("alogrithmic_model_refined.png")
